...
const creditAmount = document.querySelector(".total-amount.credit-amount");
const lastTransactionContainer = document.querySelector(
...
const TransferCloseBtn = document.querySelector(
  ".dashboard-transfer-close-btn"
);
...
class DashboardPage extends MultiPageBridge {
  ...
  async userRefreshLogin() {
    ...
    const request_connect = await fetch(
  ...
  async takeDataToLocalStorage() {
    ...
    const request_connect = await fetch(
      "http://127.0.0.1:5000/login/take_data",
      requestOptions
    );
    if (request_connect.ok) {
      let response = await request_connect.json();
  ...
  getDayName() {
    ...
    totalAmount.querySelector(
      "p"
    ).textContent = `${day_namePL}, ${date_day} ${month_namePL} ${date_year}`;
  }
  ...
  async askForHistory() {
    ...
    const request_connect = await fetch(
      "http://127.0.0.1:5000/transfers/last_history",
      requestOptions
    );
    if (request_connect.ok) {
      let response = await request_connect.json();
  ...
  async loadCreditBalance() {
    ...
    if (credit_data["STATUS"]) {
      ...
      credit_num.textContent =
        credit_data["DATA"]["credits"][0]["CREDIT_AMOUNT"];

      let credit_num_monthly = creditAmount.querySelector("h4>span");
      credit_num_monthly.textContent =
        credit_data["DATA"]["credits"][0]["CREDIT_MONTHLY"];
  ...
  async checkCredits() {
    ...
    const request_connect = await fetch(
      "http://127.0.0.1:5000/credit/havecredit",
      requestOptions
    );
    if (request_connect.ok) {
      let response = await request_connect.json();
  ...
  async takeDefoult_transactions() {
    ...
    const request_connect = await fetch(
      "http://127.0.0.1:5000/transfers/defoult",
      requestOptions
    );
    if (request_connect.ok) {
      let response = await request_connect.json();
  ...
  async MakeNewTransfer() {
    ...
    const request_connect = await fetch(
      "http://127.0.0.1:5000/transfers/newtransfer",
      requestOptions
    );
    if (request_connect.ok) {
      let response = await request_connect.json();
  ...
  async AskForCreditCost() {
    ...
    const request_connect = await fetch(
      "http://127.0.0.1:5000/credit/costs",
      requestOptions
    );
    if (request_connect.ok) {
      let response = await request_connect.json();
  ...
  async SendCreditForm(requestedAmount, monthLong) {
    ...
    const request_connect = await fetch(
      "http://127.0.0.1:5000/credit/new",
      requestOptions
    );
    if (request_connect.ok) {
      let response = await request_connect.json();
...
class DashboardInputOperator {
  ...
  AccountNumberChecker() {
    document
      .querySelector("#transfer-acc-number")
  ...
  checkLength(input, min) {
    if (input.value.length < min) {
      ...
      this.showError(
        input,
        `wartość '${inputError}' powinna składać się z min. ${min} znaków.`
      );
      return false;
    }
...
class DashboardCreator extends HTMLconstructor {
  constructor() {
    ...
    this.DashboardInputOperator = new DashboardInputOperator(
      this.DashboardPage
    );
  }
  ...
  closePopups() {
    [TransferContainer, CardsContainer, CreditContainer].forEach((container) =>
      container.classList.remove("active")
    );
    [NavDashboardPage, NavCreateTransfer, NavCardsMenagment].forEach((btn) =>
  ...
  async checkTransferData() {
    this.DashboardInputOperator.checkForm([
      TransferAccNumber,
      TransferAmount,
      TransferDate,
    ]);
  ...
  calculateCredit(CreditPopup) {
    ...
    let AdditionalConst_value =
      Number(AdditionalCostelement.split("%")[0]) * 0.01;
    ...
    if (AmountValue > 0 && MonthValue > 0) {
      let monthly_payment =
        (AmountValue *
          (month_RRSO_value * (1 + month_RRSO_value) ** MonthValue)) /
        ((1 + month_RRSO_value) ** MonthValue - 1);
  ...
  async CreditForm(CreditPopup) {
    ...
    if (AmountValue > 0 && MonthValue > 0) {
      let credit_response = await this.DashboardPage.SendCreditForm(
        AmountValue,
        MonthValue
      );
      if (credit_response["STATUS"]) {
        ...
        this.DashboardPage.loggedUserData["ACC_BALANSE"] =
